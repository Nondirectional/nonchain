...
`ElasticsearchBM25Retriever` 是基于 Elasticsearch BM25 全文检索的 `KeywordRetriever` 实现。第一版固定使用 `match` 查询检索 `content` 字段，并固定使用 `ik_smart`。
...
// 空查询返回空结果
// SearchRequest 在 build 时直接拒绝 queryText / queryEmbedding 同时为空
```

| 方法 | 说明 |
|------|------|
| `search(SearchRequest request)` | 执行 BM25 检索 |
...
推荐通过 `ElasticsearchKnowledgeStore.createBM25Retriever()` 创建 BM25 检索器，确保使用相同的索引配置：
...
第一版 BM25 路径固定使用 `ik_smart`，以减少 Elasticsearch-only 收敛阶段的配置面。
