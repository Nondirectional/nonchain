...
| 构造器 | 说明 |
|--------|------|
| `DashscopeLLM(String model)` | 使用 `DASHSCOPE_API_KEY` 环境变量 |
| `DashscopeLLM(String apiKey, String model)` | 显式传入 API Key |
...
| `maxCompletionTokens(Integer)` | `DashscopeLLM` | 最大生成 token 数 |
| `callback(ChainCallback)` | `DashscopeLLM` | 设置回调 |
...
// 方式一：fluent setter 指定
LLM llm = new DashscopeLLM("qwen-plus")
    .maxCompletionTokens(256);

// 方式二：显式传入 API Key
LLM llm2 = new DashscopeLLM("sk-xxx", "qwen-plus")
    .maxCompletionTokens(256);
```
