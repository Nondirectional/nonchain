...
`ContentPart` 是多模态消息内容部件的标记接口。目前提供三个实现类：
...
### ImageDataPart - Base64 图片数据部件

用于携带 base64 编码的图片数据，适用于本地文件或已有 base64 数据的场景（如 vLLM 本地部署）。

```java
// 方式一：直接传入 base64 数据和 MIME 类型
ImageDataPart image = ImageDataPart.of(base64String, "image/jpeg");

// 方式二：从本地文件自动读取并转 base64（自动检测 MIME 类型）
ImageDataPart image = ImageDataPart.fromFile("/path/to/photo.jpg");
...
`fromFile` 支持以下图片格式：png、jpg/jpeg、gif、webp。
...
| 模型名称 | 说明 |
|----------|------|
| `Qwen/Qwen2-VL-7B-Instruct` | Qwen2 视觉理解 7B 版本 |
| 其他视觉模型 | 任何通过 vLLM 部署且支持视觉理解的模型 |

vLLM 部署的视觉模型支持 `ImageDataPart`（base64/本地文件）和 `ImageUrlPart`（URL）两种图片输入方式，适合本地部署场景。
...
使用 vLLM 部署的视觉模型，可以直接传入本地图片文件，无需部署图片服务器：
...
public class VLLMFileImageExample {
    public static void main(String[] args) {
        LLM llm = new VLLM("http://localhost:8000/v1", "Qwen/Qwen2-VL-7B-Instruct");

        // 从本地文件创建图片部件（自动检测 MIME 类型并转 base64）
        Message userMessage = Message.user(Arrays.asList(
...
public class VLLMBase64ImageExample {
    public static void main(String[] args) {
        LLM llm = new VLLM("http://localhost:8000/v1", "Qwen/Qwen2-VL-7B-Instruct");

        String base64Data = "..."; // base64 编码的图片数据
...
- [VLLM](./vllm.md) - vLLM 推理服务器 provider
