...
通过 `ChatMemory` 接口管理多轮对话历史，与 Agent 无缝集成：

- **滑动窗口** — `MessageWindowChatMemory`：保留最近 N 条消息，保护 SystemMessage 和工具消息配对
- **Token 裁剪** — `TokenWindowChatMemory`：按 Token 数量限制上下文大小
- **多会话支持** — 通过 `conversationId` 区分不同用户/会话
- **持久化** — 内置内存存储和 MySQL 持久化（`chain-mysql` 子模块），接口可扩展（Postgres/Mongo 等）
