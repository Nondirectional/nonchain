...
│                          │                                     │
│   ┌──────────────────────┐  ┌──────────────────────┐           │
│   │ Agent / SubAgent     │  │ SkillRegistry        │           │
│   │ loop + progress event│  │ procedural knowledge │           │
│   └──────────────────────┘  └──────────────────────┘           │
│                                                                │
│   ┌──────────────────────┐                                     │
│   │ ChatMemory (策略接口) │                                     │
...
- `LLM` — 大语言模型调用接口，定义了 `chat()` / `streamChat()` 系列方法，并通过 `supportsMultipleSystemMessages(...)` 与 `prepareMessages(...)` 声明和处理 Chat Template 消息能力
...
Framework 层是 nonchain 的核心，提供 Agent 编排、过程性 Skill、工具管理、工作流编排和消息模型等基础能力。
...
- 支持注册委派型子代理（SubAgent）：`registerSubAgent(name, description)` 声明式注册，由 `Agent` 按暴露模式（`DIRECT`/`DELEGATE`）决定如何把子代理暴露给 LLM；注册项可挂载独立 `SkillRegistry`，父/子 `ChainCallback` 保持隔离

**Agent 与过程性 Skill：**

- `Agent` 负责 LLM + Tool 自动循环、前台/后台 SubAgent 编排、graceful max turns、拦截器、事件和 trace 接入
- `SkillDefinition` 是不可变的过程性知识定义；`SkillRegistry` 负责 fluent/值对象注册，并把 Skill 暴露为带 `[Skill]` 前缀的无参数 function
- Skill 点选后走独立注入路径，不经过 Tool 拦截器；`SkillInjectionMode.SYSTEM` 为默认，`USER` 模式保留 `[Skill: name]` 边界
- 父 Agent 的 `eventConsumer` 通过 `AgentEvent.SubAgentProgress` 接收子代理原始事件及调用上下文；前台调用使用独立 UUID，后台调用复用 `SubAgentRecord.id`
- SubAgent 上下文边界始终隔离父 system、过滤不可见消息，并丢弃不完整的 assistant/tool 调用组；自定义 `ContextSelector` 不能绕过这些不变量
...
| 接口 | 职责 | 实现类 |
|------|------|--------|
| `LLM` | 大语言模型调用与请求消息准备 | `DashscopeLLM`, `OpenAICompatibleLLM`, `VLLM` |
...
| `AgentEvent` | Agent/SubAgent 流式事件 | `TextDelta`, `ThinkingDelta`, `ToolCallDelta`, `ToolStart`, `ToolEnd`, `RoundStart`, `RoundEnd`, `SkillActivated`, `SubAgentProgress`, SubAgent 生命周期事件, `AgentError`, `Complete` |
...
- `SkillRegistry` 只负责 Skill 定义的注册和 schema 转换，注入由 `Agent` 执行
- `Agent` 只在请求副本上调用 `LLM.prepareMessages(...)`，原始 transcript、memory、event 和 trace 载荷保持不变
