...
- [x] **Agent 高级抽象** — 基础 Agent 自动工具循环已实现（`Agent` 类，LLM 调工具 → 观察结果 → 继续推理），并已支持委派型子代理（SubAgent：父 Agent 通过 tool calling 委派子任务给专职子代理，DIRECT/DELEGATE 双模式，父/子隔离）。ReAct Agent、Plan-and-Execute、多专家协作等高层 Agent 模式仍待实现
