...
- [x] **Agent 高级抽象** — 基础 Agent 自动工具循环与委派型 SubAgent 已实现（DIRECT/DELEGATE、前台/后台并行、自动 join、steer、resume、graceful max turns、并发控制与熔断）。0.11.0 新增 `SkillRegistry` 过程性知识注入、SubAgent Skill 预加载、`AgentEvent.SubAgentProgress` 实时内部事件，以及父 system 隔离、完整工具调用组和多 system Chat Template 请求归一化。ReAct Agent、Plan-and-Execute、多专家协作等高层 Agent 模式仍待实现
