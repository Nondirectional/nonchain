...
/**
 * {@link TraceStore} 的 PostgreSQL 持久化实现。
 *
 * <p>把执行链路的每个 span 落到 {@code trace_span} 表（schema 见 {@code trace_span.sql}），
 * 支持按 runtimeId 拉回完整 span 树。与 {@link PostgresChatMemoryStore} 同风格：
 * JDBC + DataSource，标准 SQL，异常包装为 {@link RuntimeException} + 中文消息。</p>
 *
 * <p><b>幂等写入</b>：{@code record} 在同一事务内「按 span_id 先 DELETE 再 INSERT」实现幂等
 * （重试不产生重复行），用可移植 SQL（H2/MySQL/PostgreSQL 通用）。{@code attributes} 整体序列化为
 * JSON 存 {@code attributes_json} 列。</p>
 *
 * <pre>{@code
